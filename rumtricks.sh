...
export WINEDEBUG="-all"; export RUMTRICKS_LOGFILE="$WINEPREFIX/rumtricks.log"; DOWNLOAD_LOCATION="${XDG_CACHE_HOME:-$HOME/.cache}/rumtricks"; mkdir -p "$DOWNLOAD_LOCATION"
...
# blame dxvk I guess?
wine() { "$WINE" $@; }; wine64() { "$WINE64" $@; }; wineboot() { "$WINEBOOT" $@; }; export -f wine wine64 wineboot

update() { echo -n "applying ${FUNCNAME[1]} | " && "$WINE" wineboot && "$WINESERVER" -w; }; regedit() { echo -n "adding registry | " && "$WINE" regedit "$1" & "$WINE64" regedit "$1" && "$WINESERVER" -w; }
extract() { echo -n "extracting $1 | " && tar -xvf "$1" &>/dev/null; }; applied() { echo "${FUNCNAME[1]}" >>"$RUMTRICKS_LOGFILE"; echo -n "${FUNCNAME[1]} applied | "; }
status() { [[ ! -f "$RUMTRICKS_LOGFILE" || -z "$(awk "/^${FUNCNAME[1]}\$/ {print \$1}" "$RUMTRICKS_LOGFILE" 2>/dev/null)" ]] || { echo -n "${FUNCNAME[1]} present | " && return 1; }; }
...
regsvr32() { echo -n "registering dlls | "; for i in "$@"; do "$WINE" regsvr32 /s "$i" & "$WINE64" regsvr32 /s "$i"; done; "$WINESERVER" -w; }
...
isolation() { status || return; update; cd "$WINEPREFIX/drive_c/users/${USER}" || exit; for entry in *; do if [ -L "$entry" ] && [ -d "$entry" ]; then rm -f "$entry"; mkdir -p "$entry"; fi; done
...
corefonts() { status || return; update; cp -r "$RMTCONTENT/corefonts/files/drive_c/windows/"* "$WINEPREFIX/drive_c/windows/"; regedit "$RMTCONTENT/corefonts/corefonts.reg"; applied; }
